...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          // Custom background with diagonal split
          CustomPaint(
...
class DiagonalPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.lineTo(size.width, size.height * 0.2); // Set diagonal slant just below the tagline
    ...
    path = Path();
    path.moveTo(0, size.height * 0.4); // Start from the bottom of the white background
    path.lineTo(0, size.height);
    path.lineTo(size.width, size.height);
    path.lineTo(size.width, size.height * 0.2); // End the yellow diagonal at 20% height
    path.close();
    canvas.drawPath(path, paint);
  }
